...
- A scheduled bare title is automatic-download intent unless the originating prompt explicitly says check only, magnet only, or no download. A qualified latest result must be passed through the high-level resolver with `--enqueue-qbittorrent`; accepting a result without enqueueing is an incomplete scheduled run.
