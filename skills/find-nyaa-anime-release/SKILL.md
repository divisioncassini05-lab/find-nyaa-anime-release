...
- For an old/completed or otherwise untracked TV/ONA, wording such as “第一季”, “找这一季”, or “整季” without an episode means one complete season package. Pass `--season SNN --whole-season`. Never reinterpret it as an arbitrary single episode, OVA, or side story.
...
4. For `season_batch`, keep only packages that cover the complete target season. Verify a bounded set of high-ranked candidates from their Nyaa file lists, exclude extras, and apply the size rule to every regular video file. Prefer a qualified exact-season package; only then accept a qualified multi-season collection containing the target season.
...
- **轻量观看** (`browse`): actual single-episode size `1-2 GiB`; never accept less than `1 GiB`, prefer about `1.5 GiB`.
- **普通观看** (`watch`): actual single-episode size `2-4 GiB`.
- **高画质** (`premium`): actual single-episode size at least `6 GiB`, BDMV, remux, or a comparable lossless source.
...
- Do not reduce any tier floor because an episode is short. For a season package, use the detail-page file list and require every regular video file to pass; total-size division is only an early impossibility check, never final proof.
